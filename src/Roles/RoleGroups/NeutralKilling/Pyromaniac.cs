...
public class Pyromaniac : NeutralKillingBase
{
    private static IAccumulativeStatistic<int> _dousedPlayers = Statistic<int>.CreateAccumulative($"Roles.{nameof(Pyromaniac)}.DousedPlayers", () => Translations.DousedPlayerStatistic);
    private static IAccumulativeStatistic<int> _incineratedPlayers = Statistic<int>.CreateAccumulative($"Roles.{nameof(Pyromaniac)}.IncineratedPlayers", () => Translations.IncineratedPlayerStatistic);
    ...
    {
        if (dousedPlayers.Count = dousedPlayers.count) return;
    ...
    private void PyromaniacDies() => indicators.Values.ForEach(v => v.Delete());
